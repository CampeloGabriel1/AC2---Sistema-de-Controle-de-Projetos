...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('formProjeto').addEventListener('submit', adicionarProjeto);
    document.getElementById('formVinculo').addEventListener('submit', vincularFuncionario);
    document.getElementById('formSetor').addEventListener('submit', adicionarSetor);
...
async function carregarProjetos() {
    ...
    try {
        ...
        if (!projetos || projetos.length === 0) {
            listaProjetos.innerHTML = '<div class="empty-state">Nenhum projeto cadastrado.</div>';
            projetoSelect.innerHTML = '<option value="">Nenhum projeto disponível</option>';
            return;
        }
...
async function carregarSetores() {
    ...
    try {
        ...
        if (!setores || setores.length === 0) {
            listaSetores.innerHTML = '<div class="empty-state">Nenhum setor cadastrado.</div>';
            setorSelect.innerHTML = '<option value="">Sem setor</option>';
            return;
        }

        listaSetores.innerHTML = setores.map((setor) => `
...
async function carregarFuncionarios() {
    ...
    try {
        ...
        if (!funcionarios || funcionarios.length === 0) {
            listaFuncionarios.innerHTML = '<div class="empty-state">Nenhum funcionário cadastrado.</div>';
            funcionarioSelect.innerHTML = '<option value="">Nenhum funcionário disponível</option>';
            return;
        }
        ...
        preencherSelect(funcionarioSelect, funcionarios, 'Selecione um funcionário');
    } catch (error) {
...
async function adicionarProjeto(event) {
    ...
    if (!descricao || !dataInicio || !dataFim) {
        mostrarMensagem('mensagemProjeto', 'Preencha todos os campos do projeto.', 'error');
        return;
    }

    if (new Date(dataInicio) > new Date(dataFim)) {
        mostrarMensagem('mensagemProjeto', 'A data de início não pode ser maior que a data final.', 'error');
        return;
    }
...
async function adicionarFuncionario(event) {
    ...
    if (!nome) {
        mostrarMensagem('mensagemFuncionario', 'Informe o nome do funcionário.', 'error');
        return;
    }
    ...
    try {
        ...
        mostrarMensagem('mensagemFuncionario', 'Funcionário cadastrado com sucesso.');
        await recarregarTudo();
    } catch (error) {
...
async function vincularFuncionario(event) {
    ...
    if (!idProjeto || !idFuncionario) {
        mostrarMensagem('mensagemVinculo', 'Selecione um projeto e um funcionário.', 'error');
        return;
    }

    try {
        await requestJson(`${BASE_URL}/projetos/${idProjeto}/funcionarios/${idFuncionario}`, {
            method: 'POST'
        });

        mostrarMensagem('mensagemVinculo', 'Funcionário vinculado ao projeto com sucesso.');
